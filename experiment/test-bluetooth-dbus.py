...
def pa_source_number(address):
    ...
    if pulseaudio_source == '':
        ...
        logger.debug('Cannot find pulseaudio A2DP source {}'.format(address))
        return

    # Pulseaudio source number is the first field in a \t seperated string
    pa_source_number = pulseaudio_source.split('\t')[0]
    logger.debug('Pulseaudio A2DP source {} is #{}'.format(address, pa_source_number))
...
def pa_set_volume(address, volume):
    ...
    if pa_source:
        logger.debug('Running pactl set-source-volume {} {}'.format(pa_source, format(float(volume) / VOLUME_MAX, '.2f')))
        os.system('pactl set-source-volume {} {}'.format(pa_source, format(float(volume) / VOLUME_MAX, '.2f')))
    else:
        logger.debug('Skipping volume change')
