...
import deepspeech
from onesound_audio_tools import VADAudio
import numpy as np
import pyaudio
import soundfile
...
class PorcupineDemo(Thread):
    ...
    def __init__(
        ...
        self.model = deepspeech.Model('../../../DeepSpeech/deepspeech-0.6.1-models/output_graph.tflite', 500)
        self.model.enableDecoderWithLM('../../../DeepSpeech/deepspeech-0.6.1-models/lm.binary', '../../DeepSpeech/deepspeech-0.6.1-models/trie', 0.75, 1.85)
        endtime = datetime.now()
        print('DeepSpeech loaded in', (endtime - starttime))

        
    def transcribe(self):
#        transcribed_frames = []
#        transcribed_file_count = 1
        print("Entering .transscribe(); about to open a vad_audio object")
...
#        stream_context = self.model.createStream()
 ...
 #               pcm = struct.unpack_from("h" * vad_audio.block_size_input, frame)
 #               transcribed_frames.append(pcm)
...
#                self.model.feedAudioContent(stream_context, np.frombuffer(frame, np.int16))
 ...
 #               if transcribed_frames:
 #                     recorded_audio = np.concatenate(transcribed_frames, axis=0).astype(np.int16)
 #                     soundfile.write("sndfile"+str(transcribed_file_count)+".wav", recorded_audio, samplerate=vad_audio.sample_rate, subtype='PCM_16')
 #                     transcribed_file_count += 1
                if True:
